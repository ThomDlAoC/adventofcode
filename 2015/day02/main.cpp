 ...
 *              Solutions
 **************************************/
using lwh_dim = std::tuple<int, int, int>;

std::vector<lwh_dim> parseDimensions(std::stringstream &file_content)
{
    std::vector<lwh_dim> dimensions;
    std::string line;
    while (std::getline(file_content, line))
    {
        ...
        while (std::getline(line_stream, token, 'x'))
        {
            values.push_back(std::stoi(token));
        }
        dimensions.push_back(std::make_tuple(values[0], values[1], values[2]));
    }
    return dimensions;
}
...
std::string part1(std::stringstream &file_content)
{
    auto dimensions = parseDimensions(file_content);
    int total_area = 0;
    for(const auto &[l,w,h] : dimensions){
        int lw = l*w;
        int wh = w*h;
        int hl = h*l;
        int min_area = std::min({lw,wh,hl});
        total_area += 2*lw + 2*wh + 2*hl + min_area;
    }

    return std::to_string(total_area);
}
...
std::string part2(std::stringstream &file_content)
{
    ...
    int total_ribbon = 0;
    for(const auto &[l,w,h] : dimensions){
        int min_perimeter = std::min({2*l+2*w, 2*w+2*h, 2*h+2*l});
        int volume = l*w*h;
        total_ribbon += min_perimeter + volume;
    }

    return std::to_string(total_ribbon);   
}
